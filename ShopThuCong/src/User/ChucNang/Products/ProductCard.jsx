...
export default function ProductCard({ p, onQuickView }) {
  ...
  return (
    <div className="group bg-white rounded-3xl border border-slate-200 shadow-md 
                    hover:shadow-xl hover:-translate-y-[3px] transition-all duration-200 
      ...
      >
        <div className="rounded-2xl overflow-hidden mb-4 bg-slate-50">
          <img
            src={p.ImageURL}
            alt={p.ProductName}
            className="h-48 w-full object-cover transform group-hover:scale-[1.05] transition-all duration-300"
          />
        </div>
      </Link>

      {/* TÊN */}
      <p className="font-semibold text-slate-900 text-[15px] leading-snug line-clamp-2 flex-grow">
        {p.ProductName}
      </p>

      {/* GIÁ */}
      <p className="text-rose-500 font-extrabold text-l mt-2">
      ...
      {isActive ? (
        <button
          ...
          className="mt-4 w-full py-3
                     ...
                     rounded-xl 
                     hover:bg-slate-50 hover:border-rose-400 hover:text-rose-500
        ...
        <button
          disabled
          className="mt-4 w-full py-3 bg-slate-200 text-slate-500 rounded-xl 
